...
func eval(param MalValue, replEnv *Env, env *Env) (MalValue, error) {
	...
	case MalList:
		...
		case MalSymbol:
			...
			case "eval":
				if len(rawArgs) != 1 {
					return nil, fmt.Errorf("wrong number of arguments for eval")
				}
			...
			case "let*":
				...
				for i := 0; i < len(bindings.Values); i += 2 {
					...
					val, err := eval(bindings.Values[i+1], replEnv, newEnv)
					if err != nil {
						return nil, err
					}
				...
				return eval(rawArgs[1], env, newEnv)
			case "do":
				if len(rawArgs) == 0 {
				...
				for _, arg := range rawArgs {
					val, err := eval(arg, replEnv, env)
					if err != nil {
						return nil, err
					}
					evaled = val
				}
				return eval(evaled, replEnv, env)
			case "if":
				...
				if truthy {
					return eval(rawArgs[1], replEnv, env)
				} else {
					if len(rawArgs) != 3 {
						return nil, nil
					} else {
						return eval(rawArgs[2], replEnv, env)
					}
				}
			case "fn*":
				if len(rawArgs) != 2 {
